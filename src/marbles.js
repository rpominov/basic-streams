function isDigit(x) {
  return (
    x.charCodeAt(0) >= "0".charCodeAt(0) && x.charCodeAt(0) <= "9".charCodeAt(0)
  )
}
...
const Marbles = {
  ...
  toString(marbles) {
    return marbles
      .map(x => (Value.isInstance(x) ? Value.toString(x) : Span.toString(x)))
      .join("")
  },
  fromString(str) {
    const mapFn = x => {
...
const QueueItem = {
  ...
  applyValue(item) {
    return QueueItem.map(
      marbles => Marbles.popHeadValue(item.sink, marbles),
      item,
    )
  },
  isEmpty(item) {
...
export function exec(sourcesSpec, transformer) {
  ...
  for (let key in sourcesSpec) {
    ...
    sources[key] = sink => {
      ...
      return () => {
        updateQueue(q => Queue.removeBySink(sink, q))
      }
    }
  }
