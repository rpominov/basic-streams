  ...
  return {
    add(name) {
      return sink => {
        if (sinks[name]) {
          throw new Error(
            "in pool you can't have more than one subscriber to a same stream at a same time"
          );
        }
        sinks[name] = sink;
        return () => {
...
wrap("concat", test => {
  ...
  test("disposers called properly", t => {
    t.plan(3);
    concat([() => t.calledOnce(), () => t.calledOnce(), () => t.calledOnce()])(
      noop
    )();
  });
});
...
wrap("skipDuplicates", test => {
  const lifted = s =>
    skipDuplicates((x, y) => Math.round(x) === Math.round(y), s);
...
wrap("combineArray", test => {
  test("works fine with of()", t => {
    t.plan(1);
    combineArray([of(1), of(2), of(3)])(t.calledWith([1, 2, 3]));
  });

  test("works fine with regular (not of) streams", t => {
    t.plan(4);
    const p = pool();
    combineArray([p.add("a"), p.add("b"), p.add("c")])(
      t.calledWith([1, 2, 3], [4, 2, 3], [4, 5, 3], [4, 5, 6])
    );
    p.pushTo("a", 1);
    p.pushTo("b", 2);
    p.pushTo("c", 3);
...
wrap("transduce. cat+take", test => {
  const lifted = s =>
    transduce(transducers.comp(transducers.cat, transducers.take(3)), s);
...
wrap("transduce. take+partitionAll", test => {
  const lifted = s =>
    transduce(
      transducers.comp(transducers.take(3), transducers.partitionAll(2)),
      s
    );
  test("works... (sync)", t => {
    t.plan(2);
