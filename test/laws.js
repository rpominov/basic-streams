...
testl("Monad. Left identity.", 1, t => {
  ...
  t.deepEqual(exec(({a}) => S.chain(f, S.of(a))), exec(({a}) => f(a)))
})
...
testl("Monad. Right identity.", 1, t => {
  ...
  // M.chain(M.of, u) ≡ u
  t.deepEqual(exec(({a}) => S.chain(S.of, a)), exec(({a}) => a))
})
...
testl("Chain. Derived Applicative. Identity.", 1, t => {
  ...
  // A.ap(A.of(x => x), v) ≡ v
  t.deepEqual(exec(s => ap(S.of(x => x), s.v)), exec(s => s.v))
})
...
testl("Chain. Derived Applicative. Homomorphism.", 1, t => {
  ...
  t.deepEqual(exec(() => ap(S.of(f), S.of(x))), exec(() => S.of(f(x))))
})
